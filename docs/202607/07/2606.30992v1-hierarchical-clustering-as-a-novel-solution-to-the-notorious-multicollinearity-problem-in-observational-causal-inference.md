...
title_zh: 层次聚类：解决观测因果推断中多重共线性问题的新方案
authors: "Yufei Wu, Zhiying Gu, Alex Deng, Jacob Zhu, Linsha Chen"
date: 2026-06-30
pdf: "https://arxiv.org/pdf/2606.30992v1"
tags: ["query:profile-1"]
score: 6.0
evidence: 用于观察性因果推断的方法，旨在隔离对结果的个体影响
tldr: 本研究针对观测性因果推断中长期存在的严重多重共线性问题，提出了一种创新的层次聚类解决方案。传统方法如收缩估计量虽有助于预测，但无法保留原始因果关系。作者通过将具有高度相关性的数据单元进行层次聚类聚合，有效降低了变量间的相关性。以市场营销组合模型为例，该方法成功分离了不同广告渠道的独立影响，为处理复杂因果识别问题提供了新思路。
source: arxiv
...
motivation: 针对观测数据中高度相关的自变量导致难以准确衡量其对结果独立因果影响的挑战。
method: 采用层次聚类法对地理单元进行两步聚类：先标准化去均值以消除共同趋势，再基于变量相关性进行聚合。
result: 实验证明该方法显著减轻了多重共线性，使得在贝叶斯营销组合模型中能够清晰识别各渠道的贡献。
conclusion: 层次聚类是解决因果推断中多重共线性问题的一种通用且有效的手段，具有较强的实际应用价值。
---

## 摘要
多重共线性是观测因果推断（尤其是回归分析）中长期存在的挑战——高度相关的自变量使得难以分离它们对目标结果的个体影响。虽然收缩估计量和主成分回归等常见解决方案在预测问题中很有帮助，但一个关键局限性阻碍了它们在因果推断问题中的应用，即它们无法提供原始的因果关系。为了填补这一空白，我们提出了一种创新且直观的解决方案，通过采用层次聚类来聚合数据，从而有效缓解共线性。该方法普遍适用于具有多重共线性特征的因果问题。我们通过一个营销应用来演示其工作原理及原因。不同广告渠道的支出通常表现出相关性，这使得分别衡量其影响变得极其困难。许多先前的研究建议利用细粒度的截面数据进行更好的识别，但据我们所知，没有研究明确解决多重共线性问题，而即使有细粒度数据，多重共线性也会损害因果识别。我们建议根据营销支出相关性对地理单元进行层次聚类以减少共线性，并使用聚类层级的数据实现贝叶斯营销组合模型（Bayesian Marketing Mix Model）。这种聚类分为两个步骤：首先，我们对地理层级数据进行标准化和去均值处理，以建立统一尺度并消除共同趋势；然后，我们计算成对距离以总结地理区域之间的营销支出相关性，并将具有中度到强相关性的区域进行聚类。描述性证据和回归分析均证实，这种层次聚类有效地减轻了共线性，并有助于分别识别不同营销渠道的影响。
