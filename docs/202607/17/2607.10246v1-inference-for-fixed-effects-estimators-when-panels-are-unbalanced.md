---
title: Inference for Fixed Effects Estimators when Panels are Unbalanced
title_zh: 非平衡面板数据下固定效应估计量的推断
authors: "Daniel Czarnowske, Amrei Stammann"
date: 2026-07-11
pdf: "https://arxiv.org/pdf/2607.10246v1"
tags: ["query:profile-1"]
score: 6.5
evidence: 不平衡面板数据因果识别的方法论桥梁
tldr: 本研究探讨了非平衡面板数据下双向固定效应M估计量的渐近性质。在截面和时间维度共同增长的框架下，发现原始估计量因附带参数和反馈效应（源于预定变量或选择过程）而存在偏差。为此，作者提出了一种去偏估计量，无需预先识别预定成分即可消除偏差并实现渐近正态性，为复杂选择机制下的面板数据推断提供了稳健方案。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2607.10246v1/fig-001.webp\", \"caption\": \"Table 1: Finite-sample performance of estimators for model parameters\", \"page\": 13, \"index\": 1, \"width\": 915, \"height\": 1021}]"
motivation: 解决非平衡面板数据中双向固定效应估计量因附带参数和反馈效应导致的渐近偏差问题。
method: 在N和T共同增长的框架下推导M估计量的渐近性质，并开发出一种通用的去偏估计技术。
result: 证明了反馈偏差可由预定回归变量或选择过程引起，并验证了去偏估计量在处理这些偏差时的有效性。
conclusion: 该研究为存在缺失观测值和预定变量的面板数据提供了一种无需区分预定成分的稳健推断方法。
---

## 摘要
我们在横截面单位数量和时间周期数量共同增长的渐近框架下，推导了存在缺失观测值的双向固定效应 M-估计量的渐近性质。我们允许选择过程是确定性的（以不可观测效应和初始条件为条件）、随机的或混合的，并且仅施加了条件均值限制。未经修正的估计量呈渐近正态分布，但其中心不在零点，受到附带参数偏差和反馈偏差的影响。反馈偏差可能由结果方程中的预定回归变量以及预定的选择过程引起。我们提出了去偏估计量，可以同时处理这两种来源，而无需预先知晓哪些回归变量或选择分量是预定的。

## Abstract
We derive the asymptotic properties of two-way fixed effects M-estimators with missing observations in an asymptotic framework in which the numbers of cross-sectional units and time periods grow jointly. We allow the selection process to be deterministic (conditional on the unobserved effects and initial conditions), stochastic, or mixed, and we impose only a conditional mean restriction. The uncorrected estimators are asymptotically normal but not centered at zero, suffering from incidental parameter and feedback biases. Feedback bias can be induced by predetermined regressors in the outcome equation and by a predetermined selection process. We propose debiased estimators that handle both sources without requiring knowledge of which regressors or selection components are predetermined.