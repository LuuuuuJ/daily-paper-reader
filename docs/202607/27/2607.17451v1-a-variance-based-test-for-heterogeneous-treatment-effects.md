...
tldr: 本研究针对异质性处理效应（HTE）的存在性，提出了一种基于条件平均处理效应（CATE）方差的鲁棒非参数假设检验方法。针对传统方法在零假设下渐近方差塌陷以及样本分割导致偏差无法抵消的挑战，论文创新性地提出了“折内样本分割”算法。该方法通过在互斥子样本上评估方差并耦合相同的折外干扰估计量，实现了偏差的代数抵消，确保了检验的渐近正态性和统计功效。
source: arxiv
...
motivation: 传统的异质性处理效应检验在零假设下存在渐近方差塌陷，且标准样本分割无法消除非线性平方损失带来的正则化偏差。
method: 提出一种折内样本分割算法，通过在不相交子样本上评估方差组件并共享折外干扰估计量，实现偏差的代数抵消。
result: 理论证明和仿真实验表明，该方法在保持高统计功效的同时，比现有检验方法具有更优的显著性水平控制能力。
conclusion: 该研究为检测异质性处理效应提供了一个稳健的统计工具，能够有效识别传统非参数检验可能忽略的复杂异质性。
---
