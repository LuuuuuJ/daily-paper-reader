...
score: 7.5
evidence: 具有交互固定效应的线性面板回归用于因果推断
...
## 摘要
我们分析了具有交互固定效应和预定解释变量（例如滞后因变量）的线性面板回归模型。在横截面维度和时间周期数均趋于无穷大的极限情况下，推导出了回归系数最小二乘（LS）估计量的一阶渐近理论。我们发现了LS估计量渐近偏差的两个来源：一是由于特异性误差项的相关性或异方差性引起的偏差，二是由于预定（而非严格外生）解释变量引起的偏差。我们提供了一个偏差修正的LS估计量。我们还提出了三种经典测试统计量（Wald、LR和LM检验）的偏差修正版本，并证明了它们的渐近分布为卡方分布。蒙特卡罗模拟表明，LS估计量和测试统计量的偏差修正对于有限样本量同样有效。
