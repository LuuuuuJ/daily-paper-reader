...
tldr: 本文提出了一种基于条件扩散模型的深度非参数工具变量分位数回归（IVQR）方法。该方法采用两阶段估计框架：首先利用条件扩散模型估计内生变量与结果变量的联合分布，随后通过蒙特卡洛采样和核平滑技术在深度神经网络上最小化经验风险。理论上，研究证明了估计量的超额风险界限，并针对重尾分布提供了收敛保证。实验表明，该方法在高维场景下显著优于现有非参数IVQR方法。
source: arxiv
...
motivation: 旨在解决非参数工具变量分位数回归中内生性处理困难以及高维数据下估计精度不足的问题。
method: 结合条件扩散模型进行分布估计，并利用核平滑条件矩公式与深度神经网络进行结构分位数函数学习。
result: 建立了包含重尾分布在内的端到端总变差保证和超额风险界限，且在模拟和实际数据中表现优于现有方法。
conclusion: 该方法为处理具有内生性的复杂分位数回归问题提供了一个稳健且高效的深度学习框架，尤其适用于高维和重尾数据。
---

## 摘要
本研究提出了深度非参数工具变量分位数回归（IVQR），这是一种结合了条件扩散建模与核平滑条件矩公式的两阶段估计量。在第一阶段，我们使用方差保持条件扩散模型来估计给定工具变量下结果变量和内生协变量的联合条件分布。在第二阶段，我们通过蒙特卡洛采样和指示函数的核平滑替代函数来近似条件矩算子，然后通过深度神经网络上的经验风险最小化来估计结构分位数函数。我们为所提出的估计量建立了超额风险界限，并推导了无界支撑下条件扩散模型的端到端全变差保证，明确考虑了得分估计、提前停止和离散化误差。我们的理论是在数据分布的多项式尾部包络下开发的，并连续退化到指数情形：随着尾部指数的增长，所获得的超额风险率收敛于非参数回归的极小极大最优率，因此我们的重尾理论将经典的轻尾非参数保证作为一个极限情况涵盖在内。仿真研究和实际数据应用表明，所提出的方法优于现有的非参数 IVQR 方法，且随着协变量和工具变量维度的增加，其优势变得愈发显著。
