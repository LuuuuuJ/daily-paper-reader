...
tldr: 本研究针对饱和固定效应回归中识别变异高度集中导致的高斯推断失效问题，提出了一种基于设计矩阵构建消除干扰项对比的方法。该方法无需同质性假设或对固定效应维度的限制，在对称且任意异方差的误差项下，通过符号翻转推断实现有限样本的精确推断。在双向固定效应设计中，该方法利用观测多重图的循环空间，在捕获率与粒度之间取得平衡，并在大型网络数据中展现了良好的可扩展性和有效性。
source: arxiv
...
method: 通过设计矩阵构建能够完全消除固定效应的对比量，并利用符号翻转推断在对称误差项下实现有限样本精确推断。
result: 在双向设计中，该方法通过循环打包算法提高了捕获效率，并在Grunfeld投资回归和大型雇主-雇员匹配数据中验证了其有效性。
conclusion: 该推断框架解决了识别变异集中时的统计推断难题，为复杂固定效应模型提供了一种稳健且可扩展的精确推断方案。
---
