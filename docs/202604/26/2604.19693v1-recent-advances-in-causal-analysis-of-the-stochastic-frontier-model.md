...
score: 6.0
evidence: 综述了社会科学中双重差分和工具变量等因果推断方法。
...
## 摘要
因果推断方法（工具变量、双重差分、断点回归等）是许多社会科学领域使用的主要工具。然而，这些方法在生产率和效率研究中的应用相对滞后。一个主要原因是，当研究兴趣集中在模型的误差项时，随机前沿模型的性质并不直接适用于因果框架。本文综述了尝试将随机前沿文献与因果推断方法相结合的新兴文献。我们讨论了可能与该领域应用研究人员相关的建模方法和实证问题。本综述展示了如何将该模型轻松纳入因果分析的范畴，回顾了在该领域已取得进展的现有工作，探讨了尚未解决的挑战，并讨论了核心发现。
