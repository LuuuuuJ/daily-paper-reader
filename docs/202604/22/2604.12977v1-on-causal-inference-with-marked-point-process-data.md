...
tags: ["query:q8"]
score: 7.0
evidence: 标记点过程和事件史分析的因果推断
tldr: 本研究针对标记点过程（MPP）数据定义了动态治疗方案及潜在结果，利用鞅理论提出了识别因果效应的一致性、可交换性和正值性条件。该工作成功桥接了连续时间生存分析与离散时间因果推断，并推导出通用的边际g-公式，为处理复杂随机过程数据的因果分析提供了理论框架。
source: arxiv
selection_source: fresh_fetch
motivation: 旨在解决连续时间标记点过程数据在因果推断中缺乏明确定义和识别条件的问题。
method: 基于鞅理论制定了识别假设，并定义了适用于MPP数据的动态治疗方案和潜在结果。
result: 推导出了边际g-公式，并证明了其在特定情况下与现有离散时间或生存分析结果的一致性。
conclusion: 该研究为连续时间随机过程的因果推断奠定了理论基础，有效连接了生存分析与因果推断两大领域。
---

## 摘要
我们为标记点过程（MPP）描述的数据定义了动态治疗方案及相关的潜在结果。这些定义促使了在 MPP 数据结构中识别效应所需的常用一致性、可交换性和正值性条件的 MPP 对应版本的产生。这些条件基于鞅理论制定，使我们能够为随机过程描述的数据推导出明确的识别假设。在重要的特殊情况下，这些定义和条件与已确立的离散时间结果保持一致。因此，这项工作架起了连续时间计数过程的生存（事件历史）分析与离散时间变量因果推断这两大领域文献之间的桥梁。在制定了一组识别条件后，我们推导并表征了边际 g-公式。这些 g-公式通常与相关研究中的公式不同，但在重要的特殊情况下是一致的。我们将研究结果与之前关于（计数）过程因果推断、经典生存分析文献以及离散时间因果推断文献的研究联系起来。
